  ...
  return (
    <div className="grid grid-cols-4 bg-fuchsia-400 gap-5 w-full p-8 mb-4 rounded-lg">
      {products.map((product) => (
        <div key={product.id} className="bg-white border border-gray-200 rounded-lg shadow-lg p-6 flex flex-col items-center justify-between transform transition-transform duration-300 hover:scale-105">
          <Image
            src={product.image}
            alt={product.name}
          ...
          <div className="w-full text-center">
            ...
            <p className="text-lg font-bold text-gray-800 mt-2">Precio: ${product.price.toFixed(2)}</p>
            <p className="text-sm text-gray-600 mt-1">Tallas: {product.sizes}</p>
            <p className="text-sm text-gray-600 mt-1 mb-4">Vendedor: {product.seller}</p>
