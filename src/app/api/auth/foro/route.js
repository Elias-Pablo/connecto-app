...
export async function POST(req) {
  try {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        "INSERT INTO publicaciones_foro (titulo, descripcion, id_foro, id_perfil, tiempo_creacion) VALUES (?, ?, ?, ?, NOW())",
        [titulo, descripcion, id_foro, id_perfil],
        (error, results) => {
          if (error) {
            console.error("Error al insertar publicación:", error);
            reject(new Response(JSON.stringify({ message: "Error al insertar publicación" }), { status: 500 }));
          } else {
...
export async function GET(req) {
  ...
  if (type === "categorias") {
    return new Promise((resolve, reject) => {
      connection.query(
        "SELECT id_foro, nombre FROM foro",
        (error, results) => {
          if (error) {
  ...
  if (type === "publicaciones") {
    ...
    const query = `SELECT pf.id_publicaciones, pf.titulo, pf.descripcion, pf.id_foro, pf.id_perfil,
    ...
    return new Promise((resolve, reject) => {
      connection.query(query, [categoria, categoria, limit, offset], (error, results) => {
        if (error) {
          console.error("Error al obtener publicaciones:", error);
          reject(new Response(JSON.stringify({ message: "Error al obtener publicaciones" }), { status: 500 }));
        } else {
          resolve(new Response(JSON.stringify({ publicaciones: results }), { status: 200 }));
        }
      });
    });
  }
