...
export async function POST(req) {
  try {
    const { titulo, descripcion, id_foro, id_usuario, url_imagen } = await req.json();
...
export async function GET(req) {
  ...
  return new Promise((resolve, reject) => {
    if (type === "categorias") {
      ...
      connection.query('SELECT * FROM publicaciones_foro', (error, results) => {
        if (error) {
      ...
      reject(new Response(JSON.stringify({ message: 'Tipo de solicitud inválido' }), { status: 400 }));
    }
  });
}
