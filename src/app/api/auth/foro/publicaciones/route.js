...
export async function DELETE(req) {
  try {
    ...
    if (!id_publicaciones) {
      return new Response(
        JSON.stringify({ message: 'ID de publicación requerido' }),
        { status: 400 }
      )
    }
      ...
      connection.query(
        ...
        (error, results) => {
          if (error) {
            ...
            reject(
              new Response(
                JSON.stringify({ message: 'Error al eliminar publicación' }),
                { status: 500 }
              )
            )
      ...
      connection.query(
        ...
        (error, results) => {
          if (error) {
            ...
            reject(
              new Response(
                JSON.stringify({ message: 'Error al actualizar publicación' }),
                { status: 500 }
              )
            )
          } else if (results.affectedRows === 0) {
